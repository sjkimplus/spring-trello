...
public interface KanbanRepository extends JpaRepository<Kanban, Long> {
    ...
    @Query("SELECT MAX(k.order) FROM Kanban k WHERE k.board = :board")
    Integer findMaxOrderByBoard(@Param("board") Board board);

    @Modifying
    @Query("UPDATE Kanban k SET k.order = k.order + 1 WHERE k.board = :board AND k.order BETWEEN :startOrder AND :endOrder")
